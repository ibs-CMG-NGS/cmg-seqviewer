...
## 문제 4: 연구자·프로젝트 메타데이터 부재

### 배경

랩 내부에서 동일하거나 유사한 실험 조건(예: `1D_vs_CONTROL`)으로 여러 연구자가
반복 분석을 수행할 경우, Database Browser에서 데이터셋을 구별하기 어렵다.
현재 manifest에는 연구자 정보와 프로젝트 식별자가 없어, 같은 조건명 데이터셋이
여러 개 등록되어도 누가 언제 분석한 것인지 알 수 없다.
...
| `project_name` | 연구 묶음 식별자 (논문/과제 단위) | `"2026-human-atac"`, `"mouse-timecourse-atac"` |
| `analysis_date` | 분석 실행일 (`YYYY-MM-DD`) | `"2026-06-25"` |

### 경로명 규칙 (현행 관행 표준화)

파이프라인 결과 폴더 최상위 이름에 이미 연구자·날짜 정보가 포함된 관행이 있다:

```
/2026-ljh-hiy-human-atac/      # YYYY-이니셜들-organism-type
/2025-ljh-mouse-atac/
```

이 규칙을 **공식 표준으로 유지**하면, 앱 측에서 manifest가 없을 때
경로명을 파싱하여 `researcher`와 `analysis_date`를 자동 추론하는 fallback이 가능하다.

### 목표 manifest 포맷 (전체 필드)
...
```json
{
  ...
  "padj_cutoff": 0.05,
  "log2fc_cutoff": 0.585,
  "tags": ["DA", "ATAC", "Acute_1D", "Control", "ljh", "hiy"]
}
```

> **tags에 researcher 포함 권장**: 텍스트 검색 시 연구자 이니셜로도 찾을 수 있도록 tags 배열에 researcher 값을 자동 추가한다.

### ⚠️ 앱 측 후속 작업 (backlog)

- Import 시 manifest의 `researcher`, `project_name` 필드를 읽어 DB에 저장
- 경로명 파싱 fallback (`/YYYY-이니셜-organism-type/` → researcher, analysis_date 자동 추론)
- Database Browser에 Researcher 컬럼 추가 및 텍스트 검색 대상 포함
- Import 다이얼로그에서 추론값 미리채우기 + 수동 수정 가능
...
- [ ] manifest에 `researcher`, `project_name`, `analysis_date` 필드 추가 (문제 4)
